...
def svrg_text_generation_train(net, train_dataloader, test_dataloader):
    ...
    for epoch in range(1, epochs+1):
        ...
        if epoch % 5 == 0 or len(full_gradients.keys()) == 0:
            ...
            for batch in (train_dataloader):
                ...
                with fwAD.dual_level(), torch.no_grad(), torch.cuda.amp.autocast():
                    new_state_dict = {}
                    for n, p in params_snapshot.items():
                ...
                for name, p in params_snapshot.items():
                    full_gradients[name] += (jvp * tangents[name])
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            if torch.isnan(jvp):
                print('Current weights Iteration #:', iteration, 'Skipping NaN jvp', flush=True)
                continue
            ...
            with fwAD.dual_level(), torch.no_grad(), torch.cuda.amp.autocast():
                ...
                for n, p in params_snapshot.items():
                    new_state_dict[n] = fwAD.make_dual(p, tangents[n])
                    # new_state_dict[n] = fwAD.make_dual(p, tangents_snapshot[n])
                net.load_state_dict(new_state_dict, strict=False)
                del new_state_dict
                ...
                jvp_snapshot = fwAD.unpack_dual(loss).tangent
                if jvp_range > 0.0:
                    jvp_snapshot = torch.clamp(jvp_snapshot, min=-jvp_range, max=jvp_range)
            
            for n, p in params.items():
                if p.grad == None:
                    # p.grad = ((tangents[n] * jvp) - (tangents_snapshot[n] * jvp_snapshot) + full_gradients[n]).to(p.dtype)
                    p.grad = ((tangents[n] * (jvp - jvp_snapshot)) + full_gradients[n]).to(p.dtype)
                else:
                    # p.grad += ((tangents[n] * jvp) - (tangents_snapshot[n] * jvp_snapshot) + full_gradients[n])
                    p.grad += (tangents[n] * (jvp - jvp_snapshot)) + full_gradients[n]
...
def svrg_text_classification_train(net, train_dataloader, test_dataloader):
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            perturbation_seed = epoch #random.randint(0, 2**31 - 1)
            if fixed_seed:
                torch.manual_seed(perturbation_seed)
                if torch.cuda.is_available():
                    torch.cuda.manual_seed(perturbation_seed)
            ...
            wandb.log({
                "loss": loss.item(),
                "jvp": jvp.item(),
                "jvp_snapshot": jvp_snapshot.item(),
                "gradients_mean": {
                    name: param.grad.mean().item() for name, param in net.named_parameters() if param.grad is not None 
                },
                "gradients_std": {
...
def without_jvp_snapshot_svrg_text_classification_train(net, train_dataloader, test_dataloader):
    if optimizer_name =='adamw': 
        optimizer = AdamW(net.parameters(), lr=lr)
    elif optimizer_name =='adam': 
        optimizer = Adam(net.parameters(), lr=lr)
    ...
    for epoch in range(1, epochs+1):
        ...
        ## Snapshot-based GD
        if epoch % accumulation_steps == 0 or len(full_gradients.keys()) == 0:
            print('Updating the full gradients at epoch #', epoch)
            for name, p in params.items():
                params_snapshot[name] = p.detach().clone()
                full_gradients[name] = torch.zeros_like(p)
            ...
            for name in full_gradients:
                full_gradients[name] /= valid_iteration
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            # if torch.isnan(jvp_snapshot):
            #     print('Snapshot Iteration #:', iteration, 'Skipping NaN jvp', flush=True)
            #     break
                
            # print('jvp: ', jvp,) # ', jvp snapshot: ', jvp_snapshot)
            
            # for n, p in params.items():
            #     if p.grad == None:
            #         print('tangents', tangents[n][0][0])
            #         print('tangents * (jvp - jvp_snapshot)', tangents[n][0][0] * (jvp - jvp_snapshot))
            #         print('full_gradients', full_gradients[n][0][0])
            #         print('p.grad', ((tangents[n] * (jvp - jvp_snapshot)) + full_gradients[n])[0][0], flush=True)
            #         break 
            ...
            for n, p in params.items():
                if p.grad == None:
                    # p.grad = ((tangents[n] * jvp) - (tangents_snapshot[n] * jvp_snapshot) + full_gradients[n]).to(p.dtype)
                    # p.grad = ((tangents[n] * (jvp - jvp_snapshot)) + full_gradients[n]).to(p.dtype)
                    p.grad = ((tangents[n] * jvp) + full_gradients[n]).to(p.dtype)
                else:
                    # p.grad += ((tangents[n] * jvp) - (tangents_snapshot[n] * jvp_snapshot) + full_gradients[n])
                    # p.grad += (tangents[n] * (jvp - jvp_snapshot)) + full_gradients[n]
                    p.grad += (tangents[n] * jvp) + full_gradients[n]
        ...
        losses_epoch[epoch] = accumulated_loss / sample_count
        accuracies[epoch] = accuracy
        times[end_time - start_time] = accuracy
...
def without_jvp_snapshot_reaverage_svrg_text_classification_train(net, train_dataloader, test_dataloader):
    if optimizer_name =='adamw': 
        optimizer = AdamW(net.parameters(), lr=lr)
    elif optimizer_name =='adam': 
        optimizer = Adam(net.parameters(), lr=lr)
    ...
    for epoch in range(1, epochs+1):
        ...
        if epoch % accumulation_steps == 0 or len(full_gradients.keys()) == 0:
            ...
            for batch in (train_dataloader):
                ...
                for n, g in full_gradients.items():
                    print('current grad for full_grad:', jvp * tangents[n][0][0])
                    print('running sum full_grad:', g[0][0])
                    break
        
            for name in full_gradients:
                full_gradients[name] /= valid_iteration
            for n, g in full_gradients.items():
                print('avg full_grad:', g[0][0])
                break
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            for n, p in params.items():
                if p.grad == None:
                    # p.grad = ((tangents[n] * jvp) - (tangents_snapshot[n] * jvp_snapshot) + full_gradients[n]).to(p.dtype)
                    # p.grad = ((tangents[n] * (jvp - jvp_snapshot)) + full_gradients[n]).to(p.dtype)
                    p.grad = ((tangents[n] * jvp + full_gradients[n] * valid_iteration)/(valid_iteration + 1)).to(p.dtype)
                else:
                    # p.grad += ((tangents[n] * jvp) - (tangents_snapshot[n] * jvp_snapshot) + full_gradients[n])
                    # p.grad += (tangents[n] * (jvp - jvp_snapshot)) + full_gradients[n]
                    p.grad += (tangents[n] * jvp + full_gradients[n] * valid_iteration)/(valid_iteration + 1)
...
if __name__ == "__main__":
    ...
    wandb.init(
        project='SVRGForwardModeADBeta'+dataset,
