    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            for perturbation_index in range(perturbation_count): 
                ...
                ## Normal distribution
                # tangents = {
                #     name: (masks[name] * torch.randn_like(p) * torch.sqrt(torch.tensor(perturbation_var))).to(DEVICE) 
                #     for name, p in params.items()
                # }
                tangents = {name: 
                    (top_k_percent_mask(p, 1).to(p.dtype) * torch.randn_like(p) * torch.sqrt(torch.tensor(perturbation_var))).to(DEVICE) 
                    for name, p in params.items()
                }
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            for perturbation_index in range(perturbation_count): 
                ...
                for n, p in params.items():
                    if p.grad == None:
                        p.grad = (tangents[n] * jvp).to(p.dtype)
                        # p.grad = (tangents[n] * jvp * masks[n]).to(p.dtype)
                    else:
                        p.grad += (tangents[n] * jvp).to(p.dtype)
                        # p.grad += (tangents[n] * jvp * masks[n]).to(p.dtype)
...
if __name__ == "__main__":
    ...
    key_to_track = ''
    wandb.init(
        project='SparseForwardModeADBeta'+dataset,
