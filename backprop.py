"""Centralized implementation of Reverse-mode Automatic Differentiation"""
import os
import sys
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            if iteration % accumulation_steps == 0:
                wandb.log({
                    "loss": loss.item(),
                    "gradients_mean": {
                        name: param.grad.mean().item() for name, param in net.named_parameters() if param.requires_grad 
                    },
                    "gradients_std": {
                        name: param.grad.std().item() for name, param in net.named_parameters() if param.requires_grad 
                    },
                    "gradients_norm": {
                        name: torch.norm(param.grad).item() for name, param in net.named_parameters() if param.requires_grad
                    },
                    "gradients_max": {
                        name: param.grad.max().item() for name, param in net.named_parameters() if param.requires_grad
                    },
                    "gradients_min": {
                        name: param.grad.min().item() for name, param in net.named_parameters() if param.requires_grad
                    },
...
def track_adam_efft_grad_text_generation_train(net, train_dataloader, test_dataloader):
    if optimizer_name =='adamw': 
        optimizer = AdamW(net.parameters(), lr=lr)
    elif optimizer_name =='adam': 
        optimizer = Adam(net.parameters(), lr=lr)
    ...
    for epoch in range(1, epochs+1):
        ...
        start_test_time = time.time()        
        _, accuracy, _ = text_generation_test(net, test_dataloader)
        end_test_time = time.time()        
        print(f'Epoch {epoch} test time: {end_test_time - start_test_time}')
...
def track_adam_eff_grad_text_classification_train(net, train_dataloader, test_dataloader):
    if optimizer_name =='adamw': 
        optimizer = AdamW(net.parameters(), lr=lr)
    elif optimizer_name =='adam': 
        optimizer = Adam(net.parameters(), lr=lr)
...
def text_classification_train(net, train_dataloader, test_dataloader):
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            # print('Before Loading the Inputs.', flush=True)
            # print('Memory allocated: ', torch.cuda.max_memory_allocated() / (1024 ** 3), flush=True)
            # print('Memory reserved: ', torch.cuda.max_memory_reserved() / (1024 ** 3), flush=True)
            ...
            # print('After Loading the Inputs.', flush=True)
            # print('Memory allocated: ', torch.cuda.max_memory_allocated() / (1024 ** 3), flush=True)
            # print('Memory reserved: ', torch.cuda.max_memory_reserved() / (1024 ** 3), flush=True)
            # sys.exit(0)
            ...
            if iteration % accumulation_steps == 0:
                ...
                # print('Optimizer States. Memory allocated: ', end_memory_allocated - start_memory_allocated, flush=True)    
                # print('Optimizer States. Memory reserved: ', end_memory_reserved - start_memory_reserved, flush=True)    
                # sys.exit(0)
...
if __name__ == "__main__":
    ...
    hyperparameters['checkpointing'] = True if checkpointing == "True" else False
    checkpointing = hyperparameters['checkpointing'] 
    ...
    # print('Before Loading the Model.', flush=True)
    # torch.cuda.reset_max_memory_allocated()
    # start_memory_allocated = torch.cuda.max_memory_allocated() / (1024 ** 3)
    # start_memory_reserved = torch.cuda.max_memory_reserved() / (1024 ** 3)
    ...
    ## In case of gradient checkpoint, uncomment the line below    
    if checkpointing:
    ...
    print('Trainable Param count: ', trainable_param_count, flush=True)
    
    wandb.init(
        project='Backpropagation'+dataset,
