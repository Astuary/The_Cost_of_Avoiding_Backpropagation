"""Centralized implementation of Zero-order Differentiation with Finite Differences"""
import os
import sys
...
def trace_handler(prof: torch.profiler.profile):
   ...
   file_prefix = f"{results_dir}memory_profile/"
        
   if not os.path.exists(file_prefix):
        os.makedirs(file_prefix)

   # Construct the trace file.
   prof.export_chrome_trace(f"{file_prefix}{host_name}_{timestamp}.json.gz")

   # Construct the memory timeline file.
   prof.export_memory_timeline(f"{file_prefix}{host_name}_{timestamp}.html", device="cuda:0")
   
def svrg_text_generation_train(net, train_dataloader, test_dataloader):
    ...
    for epoch in range(1, epochs+1):
        ...
        recorded_perturbation_seeds = []
        
        ## Snapshot-based GD
        if epoch % 5 == 0 or len(full_gradients.keys()) == 0:
            print('Updating the full gradients at epoch #', epoch)
            for name, p in net.named_parameters():
                if p.requires_grad:
                    params_snapshot[name] = p.detach().clone()
                    full_gradients[name] = torch.zeros_like(p)
            ...
            for batch in (train_dataloader):
                ...
                recorded_perturbation_seeds.append(perturbation_seed)
                ...
                positively_perturbed_params = {
                    name: p + (torch.randn_like(p) * perturbation_step_size).to(DEVICE) 
                ...
                with torch.no_grad(), record_function("## forward 1 ##"): # with torch.cuda.amp.autocast():
                    outputs = net(**batch_dict, use_cache = False)
                ...
                negatively_perturbed_params = {
                    name: p - (2 * torch.randn_like(p) * perturbation_step_size).to(DEVICE) 
                    for name, p in net.named_parameters() if p.requires_grad
                }
                ...
                if torch.isnan(projected_gradient):
                    print('Iteration #:', iteration, 'Skipping NaN projected_gradient', flush=True)
                    continue
                ...
                if torch.cuda.is_available():
                    ...
                    torch.cuda.manual_seed_all(perturbation_seed)
                tangents = {
                    name: (torch.randn_like(p) * perturbation_step_size).to(DEVICE) 
                    for name, p in net.named_parameters() if p.requires_grad
                }    
                
                for name, p in params_snapshot.items():
                    full_gradients[name] += (projected_gradient * tangents[name])
                del tangents
        
            print('number of batches:', len(train_dataloader))
            for name in full_gradients.keys():
                full_gradients[name] /= len(train_dataloader)
   
        ## Updated weights SGD or Adam
        for batch_index, batch in enumerate(train_dataloader):
            ...
            for perturbation_index in range(perturbation_count): 
                ...
                positively_perturbed_params = {
                    name: p + (torch.randn_like(p) * perturbation_step_size).to(DEVICE) 
                    for name, p in params_snapshot.items() if p.requires_grad
                }
                net.load_state_dict(positively_perturbed_params, strict=False)
                del positively_perturbed_params
                ...
                with torch.no_grad(), record_function("## forward 1 ##"): # with torch.cuda.amp.autocast():
                    outputs = net(**batch_dict, use_cache = False)
                    first_loss_snapshot = outputs.loss
                
                ## Current weights second_loss    
                torch.manual_seed(perturbation_seed)
                if torch.cuda.is_available():
                    torch.cuda.manual_seed(perturbation_seed)
                    torch.cuda.manual_seed_all(perturbation_seed)
                ...
                with torch.no_grad(), record_function("## forward 2 ##"): # with torch.cuda.amp.autocast():
                    outputs = net(**batch_dict, use_cache = False)
                    second_loss_snapshot = outputs.loss    
                ...
                projected_gradient_snapshot = (first_loss_snapshot - second_loss_snapshot) / (2 * perturbation_step_size)
                if projected_gradient_range > 0.0:
                    projected_gradient_snapshot = torch.clamp(projected_gradient_snapshot, min=-projected_gradient_range, max=projected_gradient_range)
                    
                if torch.isnan(projected_gradient_snapshot):
                    print('Iteration #:', iteration, 'Skipping NaN projected_gradient_snapshot', flush=True)
                    continue
                ...
                for n, p in net.named_parameters():
                    if p.requires_grad:
                        if p.grad == None:
                            p.grad = ((tangents[n] * (projected_gradient - projected_gradient_snapshot)) + full_gradients[n]).to(p.dtype)
                        else:
                            p.grad += ((tangents[n] * (projected_gradient - projected_gradient_snapshot)) + full_gradients[n]).to(p.dtype)
            ...
            if iteration % accumulation_steps == 0:
                for n, p in net.named_parameters():
                    if p.grad is not None:
                        p.grad /= (perturbation_count * accumulation_steps)
                                        
                if grad_norm_range > 0.0:
                    torch.nn.utils.clip_grad_norm_(net.parameters(), grad_norm_range)
                if grad_value_range > 0.0:
                    torch.nn.utils.clip_grad_value_(net.parameters(), grad_value_range)
                
                wandb.log({
                    "loss": loss.item(),
                    "jvp": projected_gradient.item(),
                    "gradients_mean": {
                ...
                with record_function("## optimizer ##"):
                    ...
                    if optimizer_name =='adamw' or optimizer_name =='adam':
                        wandb.log({
                            "exp_avg_mean": {
                                name: optimizer.state[param]['exp_avg'].mean().item() for name, param in net.named_parameters() if param.requires_grad
                            },
                            "exp_avg_max": {
                                name: optimizer.state[param]['exp_avg'].max().item() for name, param in net.named_parameters() if param.requires_grad
                            },
                            "exp_avg_min": {
                                name: optimizer.state[param]['exp_avg'].min().item() for name, param in net.named_parameters() if param.requires_grad
                            },
                            "exp_avg_sq_mean": {
                                name: optimizer.state[param]['exp_avg_sq'].mean().item() for name, param in net.named_parameters() if param.requires_grad
                            },
                            "exp_avg_sq_max": {
                                name: optimizer.state[param]['exp_avg_sq'].max().item() for name, param in net.named_parameters() if param.requires_grad
                            },
                            "exp_avg_sq_min": {
                                name: optimizer.state[param]['exp_avg_sq'].min().item() for name, param in net.named_parameters() if param.requires_grad
                            },
                            "random_parameter_avg_sq": optimizer.state[dict(net.named_parameters())[key_to_track]]['exp_avg_sq'][0][0],
                            "random_parameter_avg": optimizer.state[dict(net.named_parameters())[key_to_track]]['exp_avg'][0][0],
                        })
...
def svrg_text_classification_train(net, train_dataloader, test_dataloader):
    if optimizer_name =='adamw': 
        optimizer = AdamW(net.parameters(), lr=lr)
    elif optimizer_name =='adam': 
        optimizer = Adam(net.parameters(), lr=lr)
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            for perturbation_index in range(perturbation_count): 
                perturbation_seed = recorded_perturbation_seeds[batch_index] if len(recorded_perturbation_seeds) != 0 else random.randint(0, 2**31 - 1)
            ...
            if iteration % accumulation_steps == 0:
                ...
                wandb.log({
                    "loss": loss.item(),
                    "jvp": projected_gradient.item(),
                    "jvp_snapshot": projected_gradient_snapshot.item(),
                    "gradients_mean": {
                        name: param.grad.mean().item() for name, param in net.named_parameters() if param.grad is not None 
                    },
                    "gradients_std": {
                    ...
                    "random_gradient": dict(net.named_parameters())[key_to_track].grad[0][0] if dict(net.named_parameters())[key_to_track].grad is not None else 0,
                    "full_gradients_mean": {
                        name: gradient.mean().item() for name, gradient in full_gradients.items() 
                    },
                    "full_gradients_std": {
                        name: gradient.std().item() for name, gradient in full_gradients.items() 
                    },
                    "full_gradients_norm": {
                        name: torch.norm(gradient).item() for name, gradient in full_gradients.items()
                    },
                    "full_gradients_max": {
                        name: gradient.max().item() for name, gradient in full_gradients.items()
                    },
                    "full_gradients_min": {
                        name: gradient.min().item() for name, gradient in full_gradients.items()
                    },
                    "full_random_gradient": dict(full_gradients.items())[key_to_track][0][0],
                    "perturbations_mean": {
                        name: pert.mean().item() for name, pert in tangents.items()
                    },
                    "perturbations_std": {
            ...
            wandb.log({
                ...
                "random_parameter": [key_to_track][0][0],
            })
...
def grad_accumulation_text_classification_train(net, train_dataloader, test_dataloader):
    if optimizer_name =='adamw': 
        optimizer = AdamW(net.parameters(), lr=lr)
    elif optimizer_name =='adam': 
        optimizer = Adam(net.parameters(), lr=lr)
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            for n, p in net.named_parameters():
                if p.requires_grad:
                    if p.grad == None:
                        p.grad = (tangents[n] * projected_gradient).to(p.dtype)
                    else:
                        p.grad += (tangents[n] * projected_gradient).to(p.dtype)
...
def svrg_vqa_train(net, train_dataloader, test_dataloader,):
    if optimizer_name =='adamw': 
        optimizer = AdamW(net.parameters(), lr=lr)
    elif optimizer_name =='adam': 
        optimizer = Adam(net.parameters(), lr=lr)
    ...
    for epoch in range(1, epochs+1):
        ...
        start_test_time = time.time()        
        _, test_accuracy, _ = vqa_test(net, test_dataloader,)
        end_test_time = time.time()        
        print(f'Epoch {epoch} test time: {end_test_time - start_test_time}')
        
        losses_epoch[epoch] = accumulated_loss / sample_count
        accuracies[epoch] = test_accuracy
        times_accuracy[end_time - start_time] = test_accuracy
        times_loss[end_time - start_time] = accumulated_loss / sample_count
...
if __name__ == "__main__":
    ...
    parser.add_argument('--perturbation_step_size',)
    ...
    perturbation_step_size = float(args.perturbation_step_size)
    accumulation_steps = int(args.accumulation_steps)
    projected_gradient_range = float(args.jvp_range)
    ...
    hyperparameters['perturbation_step_size'] = perturbation_step_size
    hyperparameters['accumulation_steps'] = accumulation_steps
    hyperparameters['projected_gradient_range'] = projected_gradient_range
    ...
    wandb.init(
        project='SVRGZeroOrderFiniteDifferences'+dataset,
    ...
    if dataset == 'agnews':
        train_dataloader_dict, test_dataloader_dict = agnews_sequence_classification_dataloader.get_federated_datasets(
            ...
            train_client_batch_size=batch_size, # default = 40
        ...
        svrg_text_classification_train(net, train_dataloader, test_dataloader)
    elif dataset == 'boolq':
        ...
        svrg_text_generation_train(net, train_dataloader, test_dataloader)
    elif dataset == 'mmlu':
        ...
        svrg_vqa_train(net, train_dataloader, test_dataloader,)
    elif dataset == 'gqa':
