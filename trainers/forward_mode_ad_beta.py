...
def track_adam_eff_grad_gsm8k_train(net, train_dataloader, test_dataloader):
    if optimizer_name =='adamw': 
        optimizer = AdamW(net.parameters(), lr=lr)
    elif optimizer_name =='adam': 
        optimizer = Adam(net.parameters(), lr=lr)
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            for name, param in net.named_parameters():
                if param.grad is not None:
                    ...
                    if 'exp_avg' in state and 'exp_avg_sq' in state:
                        m_t = state['exp_avg']  # First moment
                        v_t = state['exp_avg_sq']  # Second moment
                        ...
                        # Effective gradient
                        effective_grad[name] = m_hat / (torch.sqrt(v_hat) + optimizer.defaults['eps'])
            ...
            wandb.log({
                ...
                "random_perturbation": dict(tangents.items())[key_to_track][0][0],
                "effective_gradients_std": {
                    name: grad.std().item() for name, grad in effective_grad.items() 
                },
                "effective_gradients_mean": {
                    name: grad.mean().item() for name, grad in effective_grad.items() 
                },
                "effective_gradients_norm": {
                    name: torch.norm(grad).item() for name, grad in effective_grad.items() 
                },
                "effective_gradients_max": {
                    name: grad.max().item() for name, grad in effective_grad.items() 
                },
                "effective_gradients_min": {
                    name: grad.min().item() for name, grad in effective_grad.items() 
                },
            })
            del tangents
        ...
        start_test_time = time.time() 
        _, accuracy, _ = gsm8k_test(net, test_dataloader, tokenizer)
        end_test_time = time.time() 
        print(f'Epoch {epoch} test time: {end_test_time - start_test_time}')
    ...
    with torch.no_grad():
        for batch_index, batch in enumerate(test_dataloader):
            ...
            for i, (extracted_answer, full_answer) in enumerate(zip(extracted_answers, predictions)):
                ...
                # if str(ground_truth.item()) in extracted_answer:
...
def track_adam_eff_grad_agnews_train(net, train_dataloader, test_dataloader):
    if optimizer_name =='adamw': 
        optimizer = AdamW(net.parameters(), lr=lr)
    elif optimizer_name =='adam': 
        optimizer = Adam(net.parameters(), lr=lr)
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            for perturbation_index in range(perturbation_count): 
                ...
                ## Normal distribution
                tangents = {
                    # name: (torch.randn_like(p) / torch.numel(p)).to(DEVICE) 
                    name: (torch.randn_like(p) * torch.sqrt(torch.tensor(perturbation_var))).to(DEVICE) 
                    for name, p in params.items()
                    # if ('qweight' not in name and 'qzeros' not in name and 'scales' not in name) 
                    # else torch.zeros_like(p, dtype=torch.float32).to(DEVICE)
                    # for name, p in net.state_dict().items()
                }
                
                ## Uniform distribution
                # tangents = {
                #     name: (torch.empty(p.shape).uniform_(-1e-4, 1e-4)).to(DEVICE) 
                #     for name, p in params.items()
                # }
                # low, high = -torch.sqrt(torch.tensor(0.3)), torch.sqrt(torch.tensor(0.3))
                # uniform_dist = torch.distributions.Uniform(low, high)
                # tangents = {
                #     name: (sample_like(p, uniform_dist, perturbation_var)).to(DEVICE) 
                #     for name, p in params.items()
                # }
                ...
                # tangents = {
                #     name: sample_like(p, laplace_dist, perturbation_var).to(DEVICE)
                #     for name, p in params.items()
                # }
                
                with fwAD.dual_level(), torch.no_grad(), torch.cuda.amp.autocast():
                # with fwAD.dual_level(), torch.cuda.amp.autocast():
                    # for k, v in net.state_dict().items():
                    #     print(k, v.shape, v.requires_grad, flush=True)
                ...
                # if len(g_norms.keys()) != 0:
                #     for p, v, g_norm in zip(params.values(), tangents.values(), g_norms.values()):
                #         if p.grad == None:
                #             p_norm = p.norm(2)
                #             print('p_norm: ', p_norm)
                #             print('g_norm: ', g_norm)
                #             # scale = p_norm / (g_norm + 1e-8)
                #             # print('scale: ', scale)
                #             p.grad = (v * jvp * p_norm / g_norm).to(p.dtype)
                #             # print(p.grad)
                #         else:
                #             p.grad += v * jvp
                # else:
        ...
        start_test_time = time.time() 
        _, accuracy, _ = agnews_test(net, test_dataloader)
        end_test_time = time.time() 
        print(f'Epoch {epoch} test time: {end_test_time - start_test_time}')
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            with fwAD.dual_level(), torch.no_grad():
                def tangent_generator(num_samples=10):
                    for _ in range(num_samples):
                        ...
                        yield tangents
                tangent_iterator = tangent_generator(2)
                
                def one_perturbation_fmad(tangents):
                    new_state_dict = {}
                    for n, p in params.items():
                        new_state_dict[n] = fwAD.make_dual(p, tangents[n])
                    net.load_state_dict(new_state_dict, strict=False)
                ...
                with ThreadPoolExecutor(max_workers=4) as executor: 
                    results = list(executor.map(one_perturbation_fmad, tangent_iterator))  
                    print(results) 
                accumulated_loss += loss.item()
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            for perturbation_index in range(perturbation_count): 
                ...
                tangents = {name: 
                    (torch.randn_like(p) * torch.sqrt(torch.tensor(perturbation_var))).to(DEVICE) 
                    for name, p in params.items()
                }
...
def get_quantized_model_path(model_name, dataset=None):
    ...
    #     size = model_name.split("-")[-1]
    #     if dataset in ["agnews"]:
    #         return f"{base_path}roberta-{size}-classification-4-class"
    #     elif dataset in ["boolq", "multirc"]:
    #         return f"{base_path}roberta-{size}-classification-2-class"
    #     return f"{base_path}roberta-{size}"
    # elif "bert" in model_name:
    #     size = model_name.split("-")[-1]
    #     if dataset in ["agnews"]:
    #         return f"{base_path}bert-{size}-classification-4-class"
    #     elif dataset in ["boolq", "multirc"]:
    #         return f"{base_path}bert-{size}-classification-2-class"
    #     return f"{base_path}bert-{size}"
    return None
...
if __name__ == "__main__":
    ...
    wandb.init(
        project='ForwardModeADBeta'+dataset,
