...
def text_generation_train(net, train_dataloader, test_dataloader):
    ...
    for epoch in range(1, epochs+1):
        ...
        masks = {
            name: top_k_percent_mask(p, 1).to(p.dtype) for name, p in net.named_parameters() if p.requires_grad
        }
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            for perturbation_index in range(perturbation_count): 
                ...
                positively_perturbed_params = {
                    name: p + (masks[name] * torch.randn_like(p) * perturbation_step_size).to(DEVICE) 
                ...
                negatively_perturbed_params = {
                    name: p - (2 * masks[name] * torch.randn_like(p) * perturbation_step_size).to(DEVICE) 
                    for name, p in net.named_parameters() if p.requires_grad
                }
                ...
                if torch.cuda.is_available():
                    ...
                    torch.cuda.manual_seed_all(perturbation_seed)
                tangents = {
                    name: (masks[name] * torch.randn_like(p) * perturbation_step_size).to(DEVICE) 
                    for name, p in net.named_parameters() if p.requires_grad
                }
...
def text_classification_train(net, train_dataloader, test_dataloader):
    ...
    for epoch in range(1, epochs+1):
        ...
        masks = {
            name: top_k_percent_mask(p, 1).float() for name, p in net.named_parameters() if p.requires_grad
        }
    ...
    for epoch in range(1, epochs+1):
        ...
        for batch_index, batch in enumerate(train_dataloader):
            ...
            for perturbation_index in range(perturbation_count): 
                ...
                ## Normal distribution
                positively_perturbed_params = {
                    name: p + (top_k_percent_mask(p, 1).to(p.dtype) * torch.randn_like(p) * perturbation_step_size).to(DEVICE) 
                    for name, p in net.named_parameters() if p.requires_grad
                }
                # positively_perturbed_params = {
                #     name: p + (masks[name] * torch.randn_like(p) * perturbation_step_size).to(DEVICE) 
                ...
                negatively_perturbed_params = {
                    name: p - (2 * top_k_percent_mask(p, 1).to(p.dtype) * torch.randn_like(p) * perturbation_step_size).to(DEVICE) 
                    for name, p in net.named_parameters() if p.requires_grad
                }
                # negatively_perturbed_params = {
                #     name: p - (2 * masks[name] * torch.randn_like(p) * perturbation_step_size).to(DEVICE) 
                ...
                if torch.cuda.is_available():
                    ...
                    torch.cuda.manual_seed_all(perturbation_seed)
                tangents = {
                    name: (top_k_percent_mask(p, 1).to(p.dtype) * torch.randn_like(p) * perturbation_step_size).to(DEVICE) 
                    for name, p in net.named_parameters() if p.requires_grad
                }
...
if __name__ == "__main__":
    ...
    hyperparameters['sparse'] = True if sparse == "True" else False
    sparse = hyperparameters['sparse'] 
    ...
    wandb.init(
        project='SparseZeroOrderFiniteDifferences'+dataset,
